<template>
	<view class="container">
		...
		<view class="height-display ">
			<text class="height-number number1-1">{{ inputHeight }}</text>
			<text class="height-unit">cm</text>
		</view>

		<!-- 身高选择器 -->
		<view v-if="inputHeight">
			<dlm-slide-choose :width="100" :bigItemWidth="5" :littleItemWidth="5" :defaultValue="defaultValue"
				@input="change" height="300rpx" :cellWidth="10" :cells="10" :cellNum="1" :startNum="50" :endNum="250"
				bgColor="linear-gradient(to right, #ea4c89, #ea4c89)" color="#fff" :bigItemHeight="40"
				:littleItemHeight="20" selColor="#0ff" align="center" numPos="top" ref="slide"></dlm-slide-choose>
		</view>

		<!-- 提交按钮 -->
		<button class="button-1" style="width:80%" @click="submitRecord">记录该身高</button>
		...
		<view style="margin: 24px;width:80%;">
			...
			<ua-timeline>
				<template v-for="(record, index) in heightRecords" :key="record.id">
	...
	export default {
		...
		methods: {
			...
			async init() {
				...
				let res = await Api.getMyHeightRecords();
				res = res.sort((a, b) => new Date(b.createTime) - new Date(a.createTime));
				this.heightRecords = res;

				this.defaultValue = this.inputHeight = Number(res[0]?.height || 150); // 取最新的身高
				setTimeout(() => {
					this.loadChart(); // 加载图表
