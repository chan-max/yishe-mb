 ...
 * @prop {OnMoveCallback}[onMove] - onMove 回调函数。你的渲染函数在这里调用。
 * @prop {OnMoveStartCallback}[onMoveStart] - 移动开始时执行的函数
 * @prop {OnMoveEndCallback}[onMoveEnd] - 移动结束时执行的函数
 * @prop {BoundsData} [bounds] - 区域的边界数据
 * @prop {PackedCircleData[]} [circles] - 圆形的数据数组
 * @prop {VectorData} [target] - 吸引目标的位置
 * @prop {boolean} [animationLoop=false] - 是否在一个持续的动画循环中更新圆形位置？
 * @prop {number} [centeringPasses=1] - 居中计算的遍历次数
 * @prop {number} [collisionPasses=3] - 碰撞检测的遍历次数
 * @prop {number} [correctionPasses=3] - 重叠修正的遍历次数
 * @prop {boolean} [calculateOverlap=false] - 是否计算圆的重叠情况
 * @prop {boolean} [noWorker=false] - 如果设置为 true，则跳过使用 Web Worker
 * @prop {boolean} [destroyAfterOneMove=false] - 如果设置为 true，则在第一次 onMove 回调后终止 worker。
 *
 * @callback OnMoveStartCallback
 * @param {CirclePackerMovementResult} updatedCircles - 包含所有圆形数据的对象
 * @returns {void}
 *
 * @callback OnMoveCallback
 * @param {CirclePackerMovementResult} updatedCircles - 包含所有圆形数据的对象
 * @param {VectorData} [target] - 吸引目标的位置
 * @param {CirclePackerOverlappingCircles} [overlappingCircles] - 包含重叠圆信息的对象
 * @returns {void}
 *
 ...
 * @prop {number} x - 向量的 X 分量
 * @prop {number} y - 向量的 Y 分量
 *
 * @typedef {number | string} CircleID
 *
 * @typedef {Object} PackedCircleData
 * @prop {CircleID} id - 圆的 ID
 * @prop {number} [x] - 圆的 X 坐标
 * @prop {number} [y] - 圆的 Y 坐标
 * @prop {VectorData} [position] - 圆的位置对象
 * @prop {number} radius - 圆的半径
 * @prop {boolean} [isPinned=false] - 圆是否被固定
 * @prop {boolean} [isPulledToTarget=false] - 圆是否被吸引到目标
 *
 * @typedef {Object} PackedCircle
 * @prop {CircleID} id - 圆的 ID
 * @prop {VectorData} position - 圆的位置
 * @prop {VectorData} [targetPosition] - 圆的目标位置
 * @prop {VectorData} previousPosition - 圆的上一次位置
 * @prop {number} radius - 圆的半径
 * @prop {boolean} [isPinned=false] - 圆是否被固定
 * @prop {boolean} [isPulledToTarget=false] - 圆是否被吸引到目标
 *
 ...
 * @prop {CircleID} id - 圆的 ID
 * @prop {VectorData} position - 圆的当前位置
 * @prop {VectorData} previousPosition - 圆的上一次位置
 * @prop {number} radius - 圆的半径
 * @prop {VectorData} delta - 圆的移动向量
 * @prop {boolean} isPulledToTarget - 圆是否被吸引到目标
 * @prop {boolean} isPinned - 圆是否被固定
 *
 * @typedef {{[key: CircleID]: PackedCircleMovementData}} CirclePackerMovementResult
 *
 * @typedef OverlapData
 * @prop {CircleID} overlappingCircleId - 重叠圆的 ID
 * @prop {number} overlapDistance - 重叠的距离（沿两个圆心连线测量）
 *
 ...
 * @prop {VectorData} position - 圆的位置
 * @prop {number} radius - 圆的半径
 * @prop {boolean} [isPinned=false] - 圆是否被固定（固定的圆不会移动）
 *
 * @typedef {CircleData | CircleID} CircleRef
 *
 * @typedef {Object} WorkerMessage
 * @prop {number} messageId - 消息的唯一 ID
 * @prop {WorkerAction} action - worker 应执行的操作
 *
 * @typedef {Object} BoundsPoints
 * @prop {VectorData} point1 - 第一个角点
 * @prop {VectorData} point2 - 第二个角点
 *
 * @typedef {Object} BoundsDimensions
 * @prop {number} width - 边界的宽度
 * @prop {number} height - 边界的高度
 * @prop {number} [x=0] - 边界的 X 坐标
 * @prop {number} [y=0] - 边界的 Y 坐标
 *
 * @typedef {Object} BoundsRect
 * @prop {number} left - 边界的左侧 X 坐标
 * @prop {number} top - 边界的顶部 Y 坐标
 * @prop {number} right - 边界的右侧 X 坐标
 * @prop {number} bottom - 边界的底部 Y 坐标
 *
 * @typedef {Object} BoundsPositions
 * @prop {number} x1 - 第一个角点的 X 值
 * @prop {number} y1 - 第一个角点的 Y 值
 * @prop {number} x2 - 第二个角点的 X 值
 * @prop {number} y2 - 第二个角点的 Y 值
 *
 * @typedef {BoundsDimensions | BoundsPoints | BoundsRect | BoundsPositions} BoundsData - 构造 Bounds 实例所需的数据
 * @typedef {Object} SetBoundsAction
 * @prop {'SET_BOUNDS'} type - 操作类型
 * @prop {BoundsData} bounds - 新的边界对象
 *
 * @typedef {Object} CenteringPassesAction
 * @prop {'SET_CENTERING_PASSES'} type - 操作类型
 * @prop {number} numberOfCenteringPasses - 新的居中计算遍历次数
 *
 * @typedef {Object} CollisionPassesAction
 * @prop {'SET_COLLISION_PASSES'} type - 操作类型
 * @prop {number} numberOfCollisionPasses - 新的碰撞检测遍历次数
 *
 * @typedef {Object} CorrectionPassesAction
 * @prop {'SET_CORRECTION_PASSES'} type - 操作类型
 * @prop {number} numberOfCorrectionPasses - 新的重叠修正遍历次数
 *
 * @typedef {Object} CalculateOverlapAction
 * @prop {'SET_CALCULATE_OVERLAP'} type - 操作类型
 * @prop {boolean} calculateOverlap - 新的是否计算重叠值
 *
 * @typedef {Object} DampingAction
 * @prop {'SET_DAMPING'} type - 操作类型
 * @prop {number} damping - 新的阻尼值
 *
 * @typedef {Object} UpdateAction
 * @prop {'UPDATE'} type - 操作类型
 *
 * @typedef {Object} TargetPullAction
 * @prop {'SET_TARGET_PULL'} type - 操作类型
 * @prop {boolean} targetPull - 新的吸引目标状态
 *
 * @typedef {Object} AddCirclesAction
 * @prop {'ADD_CIRCLES'} type - 操作类型
 * @prop {PackedCircleData[]} circles - 要添加的新圆数组
 *
 * @typedef {Object} RemoveCircleAction
 * @prop {'REMOVE_CIRCLE'} type - 操作类型
 * @prop {CircleID} id - 要移除的圆的 ID
 *
 * @typedef {Object} DragStartAction
 * @prop {'DRAG_START'} type - 操作类型
 * @prop {CircleID} id - 拖动的圆的 ID
 *
 * @typedef {Object} DragEndAction
 * @prop {'DRAG_END'} type - 操作类型
 * @prop {CircleID} id - 拖动结束的圆的 ID
 *
 * @typedef {Object} DragMoveAction
 * @prop {'DRAG_MOVE'} type - 操作类型
 * @prop {CircleID} id - 正在移动的圆的 ID
 * @prop {VectorData} position - 圆的新位置
 *
 * @typedef {Object} CircleRadiusAction
 * @prop {'SET_CIRCLE_RADIUS'} type - 操作类型
 * @prop {CircleID} id - 圆的 ID
 * @prop {number} radius - 圆的新半径
 *
 * @typedef {Object} CircleTargetPullAction
 * @prop {'SET_CIRCLE_TARGET_PULL'} type - 操作类型
 * @prop {CircleID} id - 圆的 ID
 * @prop {boolean} targetPull - 新的吸引目标状态
 *
 * @typedef {Object} PinCircleAction
 * @prop {'PIN_CIRCLE'} type - 操作类型
 * @prop {CircleID} id - 要固定的圆的 ID
 *
 * @typedef {Object} UnpinCircleAction
 * @prop {'UNPIN_CIRCLE'} type - 操作类型
 * @prop {CircleID} id - 要取消固定的圆的 ID
 *
 * @typedef {Object} SetTargetAction
 * @prop {'SET_TARGET'} type - 操作类型
 * @prop {VectorData} target - 新的吸引目标位置
 *
 ...
 * @prop {'MOVED'} type - 响应类型
 * @prop {CirclePackerMovementResult} updatedCircles - 包含所有圆形数据的对象
 * @prop {VectorData} [target] - 吸引目标的位置
 * @prop {CirclePackerOverlappingCircles} [overlappingCircles] - 包含重叠圆信息的对象
 *
 * @typedef {Object} MoveStartResponse
 * @prop {'MOVE_START'} type - 响应类型
 *
 * @typedef {Object} MoveEndResponse
 * @prop {'MOVE_END'} type - 响应类型
 * @prop {CirclePackerMovementResult} updatedCircles - 包含所有圆形数据的对象
 *
 * @typedef { MoveResponse | MoveStartResponse | MoveEndResponse } WorkerResponse
 *
 * @callback WorkerResponseCallback
 * @param {WorkerResponse} workerResponse - 通过 postMessage 发送的 worker 响应
 * @returns {void}
 *
 * @typedef {Object} PackParams
 * @prop {BoundsData} [bounds] - 区域的边界数据
 * @prop {PackedCircleData[]} [circles] - 圆形数据数组
 * @prop {VectorData} [target] - 吸引目标的位置
 * @prop {number} [centeringPasses=1] - 居中计算遍历次数
 * @prop {number} [collisionPasses=3] - 碰撞检测遍历次数
 * @prop {number} [correctionPasses=3] - 重叠修正遍历次数
 * @prop {boolean} [calculateOverlap=false] - 是否计算重叠情况
 * @prop {boolean} [noWorker=false] - 如果设置为 true，则跳过使用 Web Worker
 *
...
class PackedCircleManager {
	...
	addCircle(aCircle) {
		if (!(aCircle instanceof PackedCircle)) {
			aCircle = new PackedCircle({
				...
				isPulledToTarget: typeof aCircle.isPulledToTarget === 'boolean' ? aCircle.isPulledToTarget :
					true,
			});
		}
	...
	pushAllCirclesTowardTarget(aTarget) {
		...
		for (
			let centeringPassNumber = 0; centeringPassNumber < this.numberOfCenteringPasses; centeringPassNumber++
		) {
	...
	handleCollisions() {
		...
		// Collide circles
		for (
			let collisionPassNumber = 0; collisionPassNumber < this.numberOfCollisionPasses; collisionPassNumber++
		) {
			for (let circleAIndex = 0; circleAIndex < circleCount; circleAIndex++) {
				const circleA = circleList[circleAIndex];

				for (
					let circleBIndex = circleAIndex + 1; circleBIndex < circleCount; circleBIndex++
				) {
		...
		this.allCircles.forEach(circleA => {
			const overlappingCirclesForCircle = this.allCircles
				...
				.map(circleB => {
					...
					return {
						overlappingCircleId: circleB.id,
						overlapDistance
					};
				})
				.filter(overlapData => {
...
class CirclePackerBrowser {
	...
	stopLoop() {
		if (this.isLooping) {
			...
			this.updateListeners({
				type: 'MOVE_END',
				updatedCircles: this.lastCirclePositions
			});
