	...
	<view class="select-ruler">
		...
		<scroll-view :scroll-x="true" @scroll="handleScroll" :scroll-left="scroll_left">
			<span class="ruler-item span-item" v-for="(item, index) in (max - min + 1)">
				<span style="font-weight: bold;color:#555;">{{((index + min)/multiple).toFixed(point)}}</span>
			</span>
		</scroll-view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			handleScroll(e) {
				...
				console.log(this.number, e.detail.scrollLeft, e.detail.scrollWidth, this.max,this.min)
